...
module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    try {
      ...
      // Concurrent saves can both pass the pre-write check; re-check after writing.
      const siblingsAfter = await SiteLinkTarget.find({
        siteLink,
        isDeleted: false,
      });
      if (hasScheduleConflict(created, siblingsAfter, created.id)) {
        await SiteLinkTarget.destroyOne({ id: created.id });
        return exits.invalid(OVERLAP_ERROR);
      }
