...
const [medias, setMedias] = useState({});
  ...
  const handleImageLoad = (img, sermonId) => {
    ...
    setMedias((prev) => {
      ...
      if (cur.fit === fitting) return prev;
      return { ...prev, [sermonId]: { ...cur, fit: fitting } };
    });
  };
  ...
  return (
    <Box>
      {currentSermonNotes.map((item) => {
        const imageSrc = sermonSeriesImages[item?.sermonSeries] ||
                  item?.imageLink;

                const media = medias[item.sermonId] || {};
                const currentSrc = media.src || imageSrc || FALLBACK_IMAGE;
                  ...
                  <chakra.img
                    ...
                    onError={() => {
                      setMedias((prev) => {
                        ...
                        if (cur.src === FALLBACK_IMAGE) return prev;
                        return {
                          ...prev,
                          [item.sermonId]: { ...cur, src: FALLBACK_IMAGE },
                        };
                      });
                    }}
                    w="100%"
...
{/* Mobile: compact rows (date + saved status on their own lines) */}
                  <Box display={{ base: 'block', md: 'none' }}>
                    <Text fontSize="13px" color="gray.600" mb={1.5}>
                      <Box as="span">
                  ...
                  {/* Desktop: combined meta line + saved pill */}
      ...
      <Flex justifyContent="flex-end" alignItems="center" mt={4} gap={2}>
        <Button
          ...
          color="gray.700"
          _hover={{ bg: '#E2E8F0' }}
          _disabled={{ bg: '#EDF2F7', color: 'gray.400', cursor: 'not-allowed' }}
        >
          &lt;
        </Button>
        ...
        <Button
          ...
          color="white"
          _hover={{ bg: '#3D5CD9' }}
          _disabled={{ bg: '#DFE7FF', color: 'gray.500', cursor: 'not-allowed' }}
        >
          &gt;
        </Button>
      </Flex>
