...
// The user's snapshot decides which courses are shown (template-only or
// archived courses the user never signed up for are excluded, since we only
// map over the snapshot's courses).
//
// For courses still active in the live template, the template's values win so
// renames/updates are reflected (admins should archive a course and add a new
// one rather than mutate an existing course's info). Archived/removed courses
// keep their snapshot values from sign-up time.
const getProgressCourses = (form, classProgress) => {
  ...
  return snapshotCourses.map((snapshotCourse) => {
    const templateCourse = templateCourses.find(
      (course) =>
        course.courseId === snapshotCourse.courseId && course.isActive !== false
    );

    // Course is no longer active in the template — fall back to the values
    // recorded at sign-up so historical courses still render.
    if (!templateCourse) return snapshotCourse;

    return {
      // Spread order matters: the template lands last so the current display
      // values (name, platform, type) win over what was recorded at sign-up.
      ...snapshotCourse,
      ...templateCourse,
      // The template's link is deliberately preferred so admins can update
      // course links after sign-up; fall back to the link captured at
      // submission time when the template has none.
